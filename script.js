...
// ── State ──────────────────────────────────────────────────────────────────
const DATA_DIR = 'data/'; // Folder containing all GeoJSON/JSON data layers
...
fetch(DATA_DIR + 'l_zone_alea_inondation_debordement_s_976.json')
    .then(res => res.json())
    .then(data => {
...
fetch(DATA_DIR + 'l_glissement_terrain_s_976.json')
    .then(res => res.json())
    .then(data => {
...
const config = {
    AEF: {
        '1x':  { file: DATA_DIR + 'AEF_masked.geojson',     layer: null },
        '6x':  { file: DATA_DIR + 'AEF_masked_6x.geojson',  layer: null },
        '24x': { file: DATA_DIR + 'AEF_masked_24x.geojson', layer: null }
    },
    TESSERA: {
        '1x':  { file: DATA_DIR + 'TESSERA_masked.geojson',     layer: null },
        '6x':  { file: DATA_DIR + 'TESSERA_masked_6x.geojson',  layer: null },
        '24x': { file: DATA_DIR + 'TESSERA_masked_24x.geojson', layer: null }
    }
};
...
async function loadRisqueLayers() {
    ...
    try {
        const [resAef, resTessera] = await Promise.all([
            fetch(DATA_DIR + 'risque_AE.json'),
            fetch(DATA_DIR + 'risque_TESS.json')
        ]);
        if (resAef.ok && resTessera.ok) {
            ...
            showToast(`Files ${DATA_DIR}risque_AE.json / ${DATA_DIR}risque_TESS.json not found.`);
        }
    } catch (e) {
...
async function loadCombinaisonLayer() {
    ...
    try {
        const response = await fetch(DATA_DIR + 'lissage_combinaison2.json');
        if (response.ok) {
            const data = await response.json();
            ...
            showToast(`File ${DATA_DIR}lissage_combinaison2.json not found.`);
        }
    } catch (e) {
